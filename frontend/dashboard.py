...
# DEFAULT_COORDS = {"lat": 57.7089, "lon": 11.9746} # Gothenburg coordinates

# def display_default_map():
#     """
#     Display a default map centered on Gothenburg with no trip markers.
#     """
#     folium_map = folium.Map(location=[DEFAULT_COORDS["lat"], DEFAULT_COORDS["lon"]], zoom_start=12)
#     folium.Marker(
...
#                 st.write(f"### Resa {i + 1}")
#                 st.dataframe(
#                     trip_renamed[["Namn", "Avgångstid", "Ankomsttid", "Tid kvar (min)"]]
#                 )
#         else:
...
def main():
    ...
    # Display default map if origin and destination are not selected
    if "origin_id" not in st.session_state or "destination_" not in st.session_state:
    ...
    # If both origin and destination IDs are selected, display a dynamic map
    if "origin_id" in st.session_state and "destination_id" in st.session_state:
